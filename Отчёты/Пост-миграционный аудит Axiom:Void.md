...
| Компонент | Было (Axiom:Void) | Стало (Axiom:Void) | Статус |
|-----------|--------------|-------------------|--------|
| Агент-файлы | 5 файлов с контекстом плитки/золота | Axiom:Void, #00D4FF, веб-студия | ✅ |
| Главный гайд | `Инструкция_Axiom:Void_Guide.md` v1.0 | `Инструкция_AxiomVoid_Guide.md` v2.0 | ✅ |
| Дашборд Бизнес | "Axiom:Void — Дашборд / укладка плитки" | "Axiom:Void — Дашборд / axiom-void.dev" | ✅ |
| Дашборд Цифровой мозг | ссылка на Axiom:Void_Guide | ссылка на AxiomVoid_Guide | ✅ |
| Brain/Memories.md | основатель Axiom:Void, укладка плитки | основатель Axiom:Void, 4 услуги | ✅ |
| Shell-скрипты (бренд) | "Локальный агент Axiom:Void v4", "AI Dispatcher Axiom:Void" | "Axiom:Void v4", "AI Dispatcher Axiom:Void" | ✅ |
| Shell-скрипты (пути) | `~/vaults/Бизнес Axiom:Void/` | не изменены (намеренно) | ✅ |
| Telegram бот | "Axiom:Void AI Bot", контекст плитки | "Axiom:Void Bot", контекст веб-студии | ✅ |
| analyzer.py | "Axiom:Void (укладка плитки, мрамор)" | "Axiom:Void (веб-разработка)" | ✅ |
| Сайт index.html | Axiom:Void тематика, #d4a843 золото, Playfair Display | Axiom:Void, #00D4FF, JetBrains Mono | ✅ |
| CLAUDE.md глобальный | "Axiom:Void / плитка / #d4a843 / мрамор" | "Axiom:Void / веб / #00D4FF / минимализм" | ✅ |
| CLAUDE.md vault | "Родион / Axiom:Void" заголовок | "Родион / Axiom:Void" | ✅ |
...
Axiom:Void      — 0 (было: в заголовке, nav, секциях)
плитка      — 0
...
1. **Сайт полностью токенизирован** — нет ни одного hardcoded цвета, все через CSS-переменные. В Axiom:Void было несколько хардкодов (`#d4a843` в `.cta-float`)
...
3. **10 полноценных секций** — у Axiom:Void сайта не было секций testimonials, why-us и process. Теперь есть
...
| Что | Почему |
|-----|--------|
| `~/vaults/Бизнес Axiom:Void/` — путь vault'а | git-репо зашит, Obsidian Sync зашит |
| `~/Desktop/axiomvoid-bot/` — имя папки | Railway привязан к git remote |
| `~/Desktop/premium-tiling-website/` — имя папки | git remote, GitHub Pages |
| `VAULT_REPOS["Бизнес Axiom:Void"]` в bot.py | ключ маршрутизации vault'а |
| `com.axiomvoid.*` — launchd plists | изменение = перерегистрация всех агентов |
| Railway project ID `62add92f` | ID облачного сервиса |
| `~/.claude/.env` | токены не изменились |
| Shell-скрипты vault-пути `Бизнес Axiom:Void/` | фактические пути на диске |
...
| `axiomvoid-bot` | `f649298` | bot.py + analyzer.py context |
...
| `axiomvoid-vau` | `f9bde54` | Business Dashboard |
| `axiomvoid-vau` | `3b5847c` | Migration report final |

---

## Как откатиться если нужно

> ⚠️ ОТКАТ — только для экстренного возврата к Axiom:Void. Это потеряет все изменения миграции.
...
git -C ~/vaults/Бизнес\ Axiom:Void checkout backup-before-axiomvoid-20260527 -- Dashboard.md
git -C ~/Desktop/axiomvoid-bot checkout backup-before-axiomvoid-20260527 -- bot.py analyzer.py
```
